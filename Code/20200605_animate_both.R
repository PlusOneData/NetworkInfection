...
gridAnimPrep <- purrr::map(seq_along(test2), function(i){
  ...
  grid.newpage()
  pushViewport(viewport(x = .75, y = .5, width = .6, height = .6))
  grid.draw(a)
  popViewport()
  pushViewport(viewport(x = .25, y = .5, width = .5, height = .5))
  grid.draw(b)
  popViewport()
...
}, movie.name = '20200605_small_world.gif')
